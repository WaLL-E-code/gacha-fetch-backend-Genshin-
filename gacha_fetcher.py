...
def fetch_banner(api_base_url: str, gacha_type: str) -> list:
    ...
    while True:
        ...
        for r in data_rows:
            r["banner"]        = gacha_type
            r["banner_label"]  = GACHA_TYPES.get(gacha_type, gacha_type)
...
def dedupe_and_sort(all_rows: list) -> list:
    ...
    for r in all_rows:
        ...
        if rid and rid not in seen:
            seen.add(rid)
            out.append(r)
    out.sort(key=lambda r: (r.get("_ts", 0), r.get("id", "")), reverse=True)  # newest first
    return out
...
def fetch_all_gachas(api_base_url: str) -> dict:
    ...
    # IMPORTANT: no disk persistence here (browser will store)
